...
import { parseLesson } from "../app/lib/quizParser";
import fs from "fs";
...
try {
    const { steps } = parseLesson(content);
    ...
    steps.forEach((step, index) => {
        ...
        if (step.quiz) {
            ...
            if (step.quiz.type === 'radio') {
                console.log(`  Options: ${(step.quiz.quizBlock as any).options.join(', ')}`);
                console.log(`  Answer: ${(step.quiz.quizBlock as any).answer}`);
            } else if (step.quiz.type === 'checkList') {
                console.log(`  Correct Answers: ${(step.quiz.quizBlock as any).answers.join(', ')}`);
            }
        } else {
            console.log("  (No quiz)");
        }
